...
export default function ScoreboardPage() {
  ...
  // Fetch active contest first
  useEffect(() => {
    fetch('/api/contests/active')
      .then((r) => r.json())
      .then((d) => {
        if (d.contest) setContestId(d.contest.id)
        else setError('No active contest right now.')
      })
          ...
          <div>
            ...
            {data && (
              <p className="text-blue-200 text-xs mt-0.5">
                Synced {timeAgo(data.synced_at)} · polling every {pollInterval}s
              </p>
            )}
          </div>
